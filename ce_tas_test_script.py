...
# Path for test sets
test_j_path = "tas_test1/jobsets"

# Path for publsihing results
result_path = "tas_test1/results"

# Path for saving reduced jobsets
reduce_job_file_path = "tas_test1/results"
...
if __name__ == '__main__':
	...
	ts = ['python', 'taskgen_file.py']
	ts_run = subprocess.Popen(ts, stdout = subprocess.PIPE)
	ts_out = ts_run.stdout.read()

	print ("Communication flows generated")
	
	js = ['python', 'ttojwp.py']
	js_run = subprocess.Popen(js, stdout = subprocess.PIPE)
	js_out = js_run.stdout.read()

	print ("task sets and job sets generated")

	try:
		os.makedirs(result_path)
	except OSError:
		print("Creation of result path failed")
	else:
	...
	for u in util:
		for t in tstr:
			...
			result_csv = "first run result_U"+str(u)+"_T"+ str(t) +".csv"

			result_csv_rj = "second run" +" result_U"+str(u)+"_T"+ str(t) +".csv"
			
			rj_result_final = reduce_job_file_path+"/"+result_csv_rj
			...
			for file in dirs:
				# if fnmatch.fnmatch(file, '*.rj.csv') == True:
				if fnmatch.fnmatch(file, '*.csv') == True:	

					des_csv = des + file

					tas_analysis = [og_path, des_csv, '--counter-example', 'first-run']
					# tas_analysis = [og_path, des_csv, '--counter-example', 'second-run']
			...
			with open(rj_result_final, 'w') as csvfile:
				resultwriter = csv.writer(csvfile, delimiter=',',quotechar='|',quoting=csv.QUOTE_MINIMAL)
				resultwriter.writerow(['Filename','Schedulability','Num_jobs', 'Num_states', 'Num_edges', 'Max_width', 'Cpu_time', 'Mem_used', 'Timeout' ]) 
			...
			for file in dirs:
				...
				# if fnmatch.fnmatch(file, '*.csv') == True:	
					...
					with open(rj_result_final, 'a') as csvfile:
						# resultwriter = csv.writer(csvfile, delimiter=',',quotechar='|',quoting=csv.QUOTE_MINIMAL)
						csv.register_dialect('yimi_up', delimiter =',', quotechar='|', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
						des_writer = csv.writer(csvfile, dialect='yimi_up')
						# resultwriter.writerow([file, int(tas_analysis_out_str_split[1].replace(",","")), float(tas_analysis_out_str_split[2].replace(",","")), float(tas_analysis_out_str_split[3].replace(",","")), float(tas_analysis_out_str_split[4].replace(",","")), float(tas_analysis_out_str_split[5].replace(",","")), float(tas_analysis_out_str_split[6].replace(",","")), float(tas_analysis_out_str_split[7].replace(",","")), int(tas_analysis_out_str_split[8].replace(",","")), int(tas_analysis_out_str_split[9].replace(",","")) ]) 
